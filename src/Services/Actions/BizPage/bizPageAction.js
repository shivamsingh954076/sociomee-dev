...
export const getUserBizPage = () => {
    let user = JSON.parse(localStorage.getItem('user'));
    return function (dispatch) {
        axios.post(`https://apiserver.msgmee.com/user/getUserBusinessPages`, {} ,{headers: { Authorization: `Bearer ${user.token}`}})
            .then((res) => {
...
export const getSingleBizPage = (id) => {
    let user = JSON.parse(localStorage.getItem('user'));
    return function (dispatch) {
        axios.post(`https://apiserver.msgmee.com/bp/getpagedetails`, {id:id} ,{headers: { Authorization: `Bearer ${user.token}`}})
            .then((res) => {
